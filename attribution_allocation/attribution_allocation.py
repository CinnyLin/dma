...
# options are ['last_touch_point', 'first_touch_point', 'last_nondirect_touch_point', 'linear', 'position']
select_model_types = ['last_touch_point', 'first_touch_point',
                      'last_nondirect_touch_point', 'linear', 'position']
write_to_file = True

# ----- Import data -----
# attribution_allocation_student_data
df = pd.read_csv('final/sub_tier_spend.csv')
channel_spend = pd.read_csv('final/channel_spend.csv')
...
for model_type in select_model_types:
    ...
    for t_ind, t_row in df_convert.iterrows():
        ...
        df.loc[t_ind, list(t_credit_dict.keys())] = list(
            t_credit_dict.values())  # add credit to original dataframe
    ...
    for t_tier in df_tier_sum.index:
        for t_channel in df_CAC.index:
            ...
            t_df_CAC_colnames = [x + '_t' +
                                 str(t_tier) for x in base_col_names]
